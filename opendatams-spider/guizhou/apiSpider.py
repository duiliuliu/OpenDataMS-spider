...
reqs = [Request(
    'get', url, params=getParams(i)) for i in range(1, 10)]
...
cataSpider.run(reqs)
# 数据保存
cataSpider.write('catalog.xlsx', write_header=True)
...
class DimensionParser(HtmlParser):
    def parse(self, response):
        ...
        try:
            ...
            res = requests.get(data['ifsAddr'])
            if res.status_code == 200 and res.text:
                item['API_AVALIABLE'] = '可用'
                # 如需下载接口数据，去掉下方注释即可
                # with open(item['name']+'.'+data['supportFormat'], 'wb') as f:
                #     f.write(res.content)
            else:
